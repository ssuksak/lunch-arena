...
Deno.serve(async (req: Request) => {
  ...
  const oldBattleId = intValue(body.old_battle_id);
  ...
  if (!oldBattleId) return json({ error: "INVALID_OLD_BATTLE_ID" }, 400);
  ...
  const oldBattle = await supabase
    .from("la_battles")
    .select("id,vote_a,vote_b")
    .eq("id", oldBattleId)
    .maybeSingle();
  if (oldBattle.error) return json({ error: "OLD_BATTLE_LOOKUP_FAILED", detail: oldBattle.error.message }, 500);
  if (!oldBattle.data?.id) return json({ error: "OLD_BATTLE_NOT_FOUND" }, 404);
  if (Number(oldBattle.data.vote_a || 0) + Number(oldBattle.data.vote_b || 0) > 0) {
    return json({ error: "BATTLE_ALREADY_VOTED" }, 409);
  }

  const deleted = await supabase.from("la_battles").delete().eq("id", oldBattleId);
  if (deleted.error) return json({ error: "OLD_BATTLE_DELETE_FAILED", detail: deleted.error.message }, 500);
