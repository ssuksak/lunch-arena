# V3 TODO LIST (2026-06-27 기준)

## 완료됨

- [x] 홈 UX를 내 학교 급식 + 실시간 급식톡 중심으로 개편
- [x] 우리학교 탭 신설
- [x] 급식톡 탭으로 전체 최신 리뷰 피드 분리
- [x] 지도/배틀 탭을 하단 탭에서 제거하고 랭킹 실험실 진입으로 정리
- [x] 리뷰 카드에 학교명, 닉네임, 작성시간, 식단 정보, 별점, 메뉴 표시
- [x] 리뷰 좋아요/싫어요 토글
- [x] 리뷰 댓글 작성
- [x] 본인 리뷰 수정/삭제
- [x] 리뷰 삭제 후 같은 급식 재평가 가능 처리
- [x] 우리학교 탭 별점 선택 버그 수정
- [x] 월간 학교 참여도 랭킹 MVP
- [x] 학교별 대표 메뉴 집계 기반
- [x] ratings meal snapshot 컬럼 추가 및 백필
- [x] meals 삭제 시 ratings는 유지되도록 FK를 ON DELETE SET NULL로 변경
- [x] 학교/리뷰/댓글/반응 조회용 주요 인덱스 추가
- [x] 월간 집계 테이블 및 트리거 기반 자동 갱신
- [x] 90일 meals retention 소량 삭제 테스트
- [x] 90일 meals retention cron 등록
- [x] 최근 2개월 월간 집계 보정 cron 등록
- [x] 댓글 수정/삭제용 DB RLS 정책 준비
- [x] GitHub Pages 반영
- [x] Apps in Toss AIT 번들 재빌드 및 배포
...
- [ ] 우리학교 탭에서 대표 메뉴/참여 랭킹 영역을 더 명확하게 다듬기
- [ ] 전체 급식톡 필터 추가: 전체 / 우리학교 / 초중고 / 사진 있음
- [ ] 리뷰 작성 완료 후 피드 갱신과 완료 안내 UX 정리
- [ ] 토스 인앱 캐시 상황을 고려한 버전 표시 또는 공지 방식 정리

## P1. 다음 DB/운영 작업

- [ ] retention 첫 실행 후 90일 초과 meals 잔여 건수 점검
- [ ] 운영 안정화 후 90일 retention을 유지할지 60일로 줄일지 결정
- [ ] 댓글 수정/삭제 UI 배포 후 review_comments RLS 실제 동작 테스트
- [ ] 공개 write 정책을 Edge Function/RPC 중심 구조로 단계적 전환 설계
- [ ] ratings/review_comments/review_reactions 쓰기 API를 서버 함수로 묶는 방안 검토
- [ ] Supabase Advisor 재점검 및 신규 경고 확인
- [ ] cron 실패 감지용 간단한 운영 체크 쿼리 또는 대시보드 준비

## P2. 인증샷/사진 기능

- [ ] Supabase Storage `reviews` 버킷 정책 확정
- [ ] 사진 업로드 용량 제한 결정
- [ ] 이미지 리사이즈/압축 방식 결정
- [ ] 사진 신고/삭제 운영 정책 결정
- [ ] 사진 있는 리뷰만 모아보는 UI 추가
- [ ] 학교별 대표 인증샷/대표 메뉴 카드 실험

## P2. 보안 강화

- [ ] 현재 client user_key 기반 수정/삭제 구조의 한계 재점검
- [ ] 리뷰/댓글 수정 전용 토큰 또는 Edge Function 기반 소유 검증 설계
- [ ] batch/sync 관련 함수의 공개 실행 권한 재점검
- [ ] 악의적 리뷰/반응 반복 등록 방지 정책 검토
- [ ] 신고/차단/숨김 처리용 최소 테이블 설계

## 결정 필요

- [ ] 지도 기능을 유지할지, 실험실 기능으로만 둘지 결정
- [ ] 배틀 기능을 계속 살릴지, v3에서는 숨김/제거할지 결정
- [ ] 10대 사용자 대상 공지/업데이트 팝업 빈도 기준 결정
- [ ] 사진 기능을 무료티어 안에서 바로 켤지, 후기 단계로 미룰지 결정
- [ ] 학교 랭킹 기준을 리뷰 수 중심으로 할지, 댓글/반응/사진 가중치를 유지할지 결정
